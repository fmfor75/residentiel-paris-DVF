...
def parse_txt_pipe(lines, annee):
    ...
    for line in lines[start:]:
        ...
        # Priorité surface : Carrez lot1 > Carrez lot2 > surface réelle bâtie
        # Dans le TXT DGFiP historique, la Carrez est la donnée fiable pour les apparts
        c1   = to_f(g(idx['c1']))   if idx['c1']   >= 0 else 0
        c2   = to_f(g(idx['c2']))   if idx['c2']   >= 0 else 0
        bati = to_f(g(idx['surf'])) if idx['surf'] >= 0 else 0
        surf = c1 if c1 > 0 else (c2 if c2 > 0 else bati)
